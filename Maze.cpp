...
void Maze::gridmaker(int m_gridSize, int windowWidth, int windowHeight, SDL_Renderer *m_renderer)
{
    ...
    for (int i = 0; i < gridSize; ++i)
    {
        ...
        for (int j = 0; j < gridSize; ++j)
        {
            grids[i][j].seti ( i);
            grids[i][j].setj ( j);
            grids[i][j].setx  (i * this->scale);
            grids[i][j].sety ( j * this->scale);
            grids[i][j].setscale ( this->scale);
...
std::vector<std::vector<int>> Maze::get_all_neighbors(int i, int j)
{
    ...
    if ((i >= 0 && i < gridSize) && (j + 1 >= 0 && j + 1 < gridSize) && (!grids[i][j + 1].getvisited()))
    {
    ...
    if ((i - 1 >= 0 && i - 1 < gridSize) && (j >= 0 && j < gridSize) && (!grids[i - 1][j].getvisited()))
    {
...
void Maze::generate_maze()
{
    ...
    grids[current_cell->geti()][current_cell->getj()].setvisited ( true);
    ...
    { // while stack is not empty
        ...
        std::vector<std::vector<int>> neighbors = get_all_neighbors(current_cell->geti(), current_cell->getj());
        int random_int = 0;

        if (!neighbors.empty())
        { // randomly select the a neighbor from the vector of neigbors
            ...
            std::string direction = grids[current_cell->geti()][current_cell->getj()].direction(*neighbor);
            // breaking wall between the neighbor and the current cell
            if (direction == "N")
            {
                grids[current_cell->geti()][current_cell->getj()].setnorthwalls( false);
            }
            else if (direction == "S")
            {
                grids[current_cell->geti()][current_cell->getj()].setsouthwalls ( false);
            }
            else if (direction == "W")
            {
                grids[current_cell->geti()][current_cell->getj()].setwestwalls(false);
            }
            else if (direction == "E")
            {
                grids[current_cell->geti()][current_cell->getj()].seteastwalls(false);
            }

            // breaking the wall from the pov of neighbor cell
            std::string direction_neighbor = grids[current_cell->geti()][current_cell->getj()].directionopposite(direction);

            if (direction_neighbor == "N")
            {
                grids[neighbor->geti()][neighbor->getj()].setnorthwalls ( false);
            }
            else if (direction_neighbor == "S")
            {
                grids[neighbor->geti()][neighbor->getj()].setsouthwalls ( false);
            }
            else if (direction_neighbor == "W")
            {
                grids[neighbor->geti()][neighbor->getj()].setwestwalls (false);
            }
            else if (direction_neighbor == "E")
            {
                grids[neighbor->geti()][neighbor->getj()].seteastwalls(false) ;
            }
            // marking the neighbor visited and pushing it into stack
            grids[a][b].setvisited  (true);
    ...
    {
        grids[exit_x][exit_y].seteastwalls(false) ;
    }
...
Maze::~Maze()
{
    std::cout << "mazed destrct";
    for (int i = 0; i < list.size(); i++)
    {
        delete grids[list[i][0]][list[i][1]].collectible;
        grids[list[i][0]][list[i][1]].collectible = nullptr;
    }
}
