...
Cell::~Cell()
{ // cell destructor, releasing dynamic memory
    this->coinimg.destroy();
    this->diamondimg.destroy();
    this->vwall.destroy();
    this->hwall.destroy();
    SDL_DestroyRenderer(renderer);
    renderer=NULL;
    std::cout << "cell destrct" << std::endl;
    delete collectible;
    collectible = nullptr;
}
...
{ // returns the direction of the other cell with respect to the cell through which it is called
...
{ // returns opposite of the direction passed
    if (direct == "W")
    {
        return "E";
    }
...
void Cell::drawCell()
{ // draws the walls around the cells according to the size of the maze using the scale
    // hwalls represent horizontal walls and vwalls represent vertical walls
    SDL_SetRenderDrawColor(this->renderer, 255, 255, 255, 0);
    if (vwall.texture == nullptr || hwall.texture == nullptr)
    {
        std::cout << "Error";
    }
    if (eastwalls == true)
    ...
    if (westwalls == true)
    {
        ...
        { // condition for opening the destination wall
...
{ // function to load textures for walls and collectibles
...
bool &Cell::operator==(Coin c)
{
    return this->haveCoin;
}
bool &Cell::operator==(Diamond d)
{
